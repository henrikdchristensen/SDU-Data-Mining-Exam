...
## Table of Contents
 ...
 1. [Data Set Generation](#data-set-generation)
 2. [ELKI](#elki)
 3. [GPUMAFIA](#gpumafia)
 4. [Helper Scripts](#helper-scripts)
 5. [Evaluation report](#evaluation)
...
### ID column
Note that the ID column first resets after closing the GUI. That is, the generation of ID's are starting from the end of the last session.
...
Combine multiple cluster output files generated by GPUMAFIA into a single output file, including non-clustered data points labeled as "noise". The resulting file can be used, for example, in ELKI. Simply place this Python script in the same directory as the output files and run it:
```bash
python compose_outputs_mafia.py
```

### count_labels_elki.py
Sometimes it is nice to be able to count the number of labels in each cluster generated in ELKI.
This script does that, simply:
```bash
python helpers/count_labels_elki.py <data-file>
```

### count_labels_mafia.py
Similarly to `count_labels_elki`, just ùsing the composed file using `compose_outputs_mafia` of the files generated by GPUMAFIA:
```bash
python helpers/count_labels_mafia.py <data-file>
```

### count_lines.py
Count the number of lines (e.g. data points) in a file:
```bash
python helpers/count_lines.py <data-file>
```

### fix_labels_ac.py
Normalize the data generated by Artificial Cluster, fix the labels and add percentage of noise:
```bash
python helpers/fix_labels_ac.py <data-file> <noise_percentage>
```
An optional `-l` flag can be added to include the true labels in the output file. 

### normalize_data.py
If using datasets that are not normalized by default, then you can normalize the data using this script, assumes txt files. It will automatically output a normalized file in the same directory as the input file.
```bash
...
1. Install a LaTeX distribution.
   ...
   - Mac: MacTeX
   - Linux: TeX Live
2.  Install pandoc (https://pandoc.org/), make sure it can be found using `pandoc --version`. Otherwise, try to re-open terminal (in VScode you need to close it entirely).
